    ...
    <div className="flex flex-wrap flex-col sm:flex-row gap-5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col justify-center items-center text-white ">
                      {user.firstname === "" && user.lastname === "" ? (
                        ...
                        <h3 className="text-[.9rem]">{`${user.firstname} ${user.middlename} ${user.lastname}`}</h3>
                      )}
                      <p class="text-yellow-300 text-[.7rem]">Employee Name</p>
                      {user.schoolid === "" ? (
                        <p class="text-white text-[.9rem]">N/A</p>
                      ) : (
                        <p className="text-[.9rem]">{user.schoolid}</p>
                      )}
                      <p class="text-yellow-300 text-[.7rem]">ID Number</p>
                      {user.position === "" ? (
                        <p class="text-white text-[.9rem]">N/A</p>
                      ) : (
                        <p className=" text-[.9rem]">{user.position}</p>
                      )}
                      ...
                      <p class="text-yellow-300 text-[.7rem]">Designation</p>
                    </div>
                  </div>
                </>
              ) : (
                ...
                <>
                  <div className="flex flex-col justify-center items-center custom-bg text-black h-[146px] ">
                    <div className="text-content">
                      ...
                      <p className="text-black text-[.7rem]">EMPLOYEE NAME</p>
                      {user.schoolid === "" ? (
                        <p className="text-[.9rem]">N/A</p>
                      ) : (
                        <p className="uppercase text-[.9rem] font-bold">
                      ...
                      <p className="text-black text-[.7rem]">EMPLOYEE NUMBER</p>
                      {user.position === "" ? (
                        <p className="text-[.9rem]">N/A</p>
                      ) : (
                        <p className="uppercase text-[.9rem] font-bold">
                ...
                <>
                  <div className="border border-orange-500 flex flex-col items-center justify-center m-2 bg-white font-bold">
                    {user.firstname === "" && user.lastname === "" ? (
                      ...
                      <h3 className="name text-center">{`${user.firstname} ${user.middlename} ${user.lastname}`}</h3>
                    )}

                    {user.schoolid === "" ? (
                      <p className="school-id">N/A</p>
                    ) : (
                      <p className="school-id">{user.schoolid}</p>
                    )}
                    ...
                    {user.role === "student" ? (
                      <>
                        {user.course === "" ? (
                          ...
                          <p className="program">{user.course}</p>
                        )}
                      </>
                    ) : (
                      <></>
      ...
      <Card maxW="sm" border="2px" borderRadius="lg" w="207px" h="326px">
        {user.role === "student" ? (
            ...
            <CardBody px={2}>
              ...
              <div className="border-2 border-black " id="case-emergency">
                ...
                <div className="px-1 text-[.7rem] ">
                  ...
                  <div className="flex gap-1 font-bold ">
                    <p>Address: </p>
                    <p className="text-[.6rem]">{user.address || "N/A"}</p>
                  </div>
                  <div className="flex gap-1 font-bold">
                    <p>Tel. No.: </p>
                    <p className="text-[.6rem]">
          ...
          <CardBody px={2}>
            <div
              className="grid grid-cols-3 border-2 border-black w-full text-[.5rem] "
            ...
            <div
              className="border-2 border-black font-[600] h-[80px]"
              id="personnel-back-card"
            >
              ...
              <div className="px-1 text-[.6rem]">
                <div className="flex gap-2">
                  <p>Name: </p>
                  <p className="text-[.6rem]">{user.contactperson || "N/A"}</p>
                </div>
                <div className="flex gap-2">
                  <p>Address: </p>
                  <p className="text-[.6rem]">{user.address || "N/A"}</p>
